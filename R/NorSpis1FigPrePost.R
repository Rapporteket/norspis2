...
NorSpis1FigPrePost <- function(RegData, reshID, outfile='', hentData=0, preprosess=1,
                              enhetsUtvalg=1,
                              valgtMaal='Gjsn',
                              valgtVar,
                              datoFra='2012-04-01', datoTil='2050-12-31',
                              datoFraSluttreg='', datoTilSluttreg='',
                              minbmistart=0, maxbmistart=200,
                              minald=0, maxald=130,
                              erMann='',
                              regType='',
                              diagnose ='') #OBS datoFraSluttreg og datoTilSluttreg må sendes inn tomme,
  #ellers så vil filteret kjøres i utvalgsfila slik at kun sluttreg (med tilhørende startreg kommer med)
  #Parametrene (datoFraSluttreg og datoTilSluttreg) er kun med fordi de de kreves å inngå i utvalgsfila,
  #som er felles for alle figurtypene. Men de må altså her settes tomme.
{
  ...
  NorSpisVarSpes <- NorSpis1_2_VarTilrettelegg(RegData=RegData, valgtVar=valgtVar)
  ...
  NorSpisUtvalg <- NorSpis1_3_Utvalg(RegData=RegData,
                                 ...
                                 datoFraSluttreg=datoFraSluttreg, datoTilSluttreg=datoTilSluttreg,
                                 aar=aar,
                                 minald=minald, maxald=maxald,
                                 minbmistart = minbmistart, maxbmistart = maxbmistart,
                                 erMann=erMann, regType=regType, reshID=reshID, enhetsUtvalg=enhetsUtvalg,
                                 diagnose=diagnose)
  ...
  #VELGER hvilke reg-typer som skal ligge i de ulike gruppene (before/pre og after/post).
  after <- RegData[which(RegData$RegRegtype %in% c(5,6,98,99)), ]
  FIDBefore <- after$RegTilhorendeStartReg #(vector of) forlopsID of the patients start reg. (of patients that also is in after group)
  before <- RegData[which(RegData$RegRegtype %in% c(1,2,3,4)) , ]
  before <- before[which(before$ForlopsID %in% FIDBefore) , ] #before with only patients from after group

  #TEST if same patients in borth groups: before$PasientID %in% after$PasientID
  #TEST if size of before and after grop is the same: length(before$PasientID) == length(after$PasientID)

  #REMOVE patients that have a missing score, on chosen variable, in either before or after group
  ...
  ind <- list(Pre = which(RegData$ForlopsID %in% FIDBefore), Post = which(RegData$ForlopsID %in% FIDAfter))

  N$Pre <- length(ind$Pre)
  #Gj.snittsverdi PRE #FIKSE:bruke mean() i stedet? Kan også beregne gj.snitt bare en gang, ikke igjen i "tekst" under.
  AggVerdier$Pre <- sum(as.numeric(as.character(before$VariabelGj)))/N$Pre
  #Gj.snittsverdi POST #FIKSE:bruke mean() i stedet? Kan også beregne gj.snitt bare en gang, ikke igjen i "tekst" under.
  ...
  TotSkaar$Pre <- sprintf('%.1f', mean(as.numeric(as.character(RegData[ind$Pre, valgtVar]), na.rm = T)))
  TotSkaar$Post <- sprintf('%1f', mean(as.numeric(as.character(RegData[ind$Post, valgtVar], na.rm = T))))

  grtxt <- c('Start','Slutt') #NorSpisVarSpes$grtxt
  grtxt2 <- ''#c(sprintf('%.1f', AggVerdier$Pre),' / ', sprintf('%.1f',AggVerdier$Post),'%')
  #grtxt2[match('', grtxt)] <- ''
  ...
  if (NPre < 5 | NPost < 5){    #KAn endres til 10 hvis ønskelig, eller 0 under testing
    ...
    pos <- barplot(t(AndelerPP), beside=TRUE, las=1, ylab="Skåre (gjennomsnitt)",
                   cex.names=0.8, col=farger[c(1,3)], names.arg=grtxt, border='white', ylim=c(0, ymax), space = c(0.05,1))	# names.arg=rep('', length(grtxt))
    ...
    #mtext(at=pos, grtxt2, side=1, las=1, cex=0.75, adj=0.2, line=0, font = 2) #opprinnelig line=0.5

    #grtxt <- delTekst(grtxt, 13)
    #mtext(at=pos[1,], grtxt, side=1, las=1, cex=0.75, adj=0.2, line=2)
    legend('top', c(paste0('Ved start, N=', NPre), paste0('Gj.sn. skår: ', round(as.numeric(as.character(TotSkaar$Pre)), digits=2)),
                    paste0('Ved slutt, N=', NPost), paste0('Gj.sn. skår: ',round(as.numeric(as.character(TotSkaar$Post)), digits=2))),
    ...
    #SE <- tapply(as.numeric(as.character(RegData$VariabelGj)), RegData[ ,grVar], sd, na.rm=T)/sqrt(Ngr)
    SEpre <- sd(as.numeric(as.character(before$VariabelGj)), na.rm=T)/sqrt(length(before$VariabelGj))
    SEpost <- sd(as.numeric(as.character(after$VariabelGj)), na.rm=T)/sqrt(length(after$VariabelGj))
    SE <- c(SEpre,SEpost)
    # Gjsn[indGrUt] <- dummy0
    # SE[indGrUt] <- 0
    ...
    KIopp <- Gjsn + 1.96*SE #FIKS/UNDERSØK: 2 eller 1,96 (noen bruker avrunding til 2 for enkelthets skyld)
    KIned <- Gjsn - 1.96*SE #FIKS/UNDERSØK: 2 eller 1,96 (noen bruker avrunding til 2 for enkelthets skyld)
    ...
    arrows(x0=posKI, y0=Gjsn, x1=posKI, y1=KIopp,  #x0=Midt[-indGrUtPlot]*0.999, x1=KIopp[-indGrUtPlot]
           length=0.5/max(pos), code=2, angle=90, lwd=1, col='black')#col=farger[1]
    # #nedre KI
    arrows(x0=posKI, y0=Gjsn, x1=posKI, y1=KIned, #y0=Midt[-indGrUtPlot]*1.001, x1=posKI, y1=KIned[-indGrUtPlot]
           length=0.5/max(pos), code=2, angle=90, lwd=1, col='black')#col=farger[1]
