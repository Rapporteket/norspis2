...
make_figFig_unitCompar <- function(my_proptable_hospitals,
  ...
  tema$plot.title = element_text(vjust=3)
  ...
  fig <-
    ggplot(my_proptable_hospitals)+
    #The bars:
    geom_col(mapping=aes(x = AvdNavn, y=perc),#,width=1/3
             fill=factor(ifelse(stringr::str_detect(my_proptable_hospitals$AvdNavn, "Nasjonal"),"grey", "#2171b5"))) +

    labs(y=my_y_lab, #my_proptable_hospitals$my_y_lab,#
         title = my_title)+#colnames(my_proptable_hospitals[,4]))+ #my_figText$title)+
    scale_y_continuous(limits = c(0,max(my_proptable_hospitals$perc)), #c(0,105)
                       expand = expansion(mult=c(0,0.1)),breaks = seq(0,100,10))+ #in "expansion use "add" instead of "mult" to make expansion abolute rather than relative(mult for multiplication)
    xlab(NULL)+
  ...
  if (GreenGoal!=''){
    if(GreenGoal=='mean'){
      fig <-   fig +
        geom_hline(yintercept = my_proptable_hospitals[stringr::str_detect(my_proptable_hospitals$AvdNavn, "Nasjonal"),]$perc, col='forestgreen', size=1, linetype=1) #to set goal value at national value: "my_proptable_hospitals[stringr::str_detect(my_proptable_hospitals$AvdNavn, "Nasjonal"),]$perc"

    } else {
      fig <-   fig +
        geom_hline(yintercept = as.numeric(GreenGoal), col='forestgreen', size=1, linetype=1)
    }
  }
  # yellow goal line:
  if (YellowGoal!=''){
    fig <-   fig +
      geom_hline(yintercept = as.numeric(YellowGoal), col='gold', size=1, linetype=1) #
