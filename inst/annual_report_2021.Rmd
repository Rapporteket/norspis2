...
if (!dir.exists("~/.ssh/norspisdata_aarsrapp2021/")) {
  dir.create("~/.ssh/norspisdata_aarsrapp2022/")
}
#------DATA norspis2
...
tabBodo_n <- 
  KI1_time[[6]] %>% 
  filter(AvdNavn == "Bodo (regional, V)") %>%
  mutate(AvdNavn = case_when(AvdNavn == "Bodo (regional, V)" ~ "bodo_n"))

tabBodo_perc <-
  KI1_time[[7]] %>% 
  filter(AvdNavn=="Bodo (regional, V)") %>%
  mutate(AvdNavn = case_when(AvdNavn == "Bodo (regional, V)" ~ "bodo_perc")
  )
