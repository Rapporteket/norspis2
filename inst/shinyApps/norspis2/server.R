...
server <- function(input, output, session) {
  ...
  ###---PDF-report, minimal for testing
...
server <- function(input, output, session) {
  ...
  ###-----PDF report for hospital unit
  output$downloadReportHospitalUnit <- downloadHandler(
    filename = function() {
      paste('norspis-periodisk-rapport', sep = '.', switch(
        input$formatReportHospitalUnit, PDF = 'pdf', HTML = 'html', Word = 'docx'
      ))
    },
    ...
    content = function(file) {
      ...
      src <- normalizePath('norspis-periodisk-rapport.Rmd')

      # temporarily switch to the temp dir, in case you do not have write
      # permission to the current working directory
      owd <- setwd(tempdir())
      on.exit(setwd(owd))
      file.copy(src, 'norspis-periodisk-rapport.Rmd', overwrite = TRUE)

      library(rmarkdown)
      out <- render(input = 'norspis-periodisk-rapport.Rmd',
                    output_format =switch(
                      input$formatReportHospitalUnit,
                      PDF = pdf_document(), HTML = html_document(), Word = word_document()),
                    params = list(dateFrom = input$dateFromReportHospitalUnit,
                                  dateTo = input$dateToReportHospitalUnit)
      )
      file.rename(out, file)
    }
  )
  ###-----PDF report for hospital unit (END)


  ###-----PDF report fagraad
  output$downloadReportFagraad <- downloadHandler(
    filename = function() {
      paste(input$fileReportFagraad, sep = '.', switch(
        input$formatReportFagraad, PDF = 'pdf', HTML = 'html', Word = 'docx'
      ))
    },
    ...
    content = function(file) {
      ...
      src <- normalizePath(paste0(input$fileReportFagraad,'.Rmd'))

      # temporarily switch to the temp dir, in case you do not have write
      # permission to the current working directory
      owd <- setwd(tempdir())
      on.exit(setwd(owd))
      file.copy(src, paste0(input$fileReportFagraad,'.Rmd'), overwrite = TRUE)

      library(rmarkdown)
      out <- render(input = paste0(input$fileReportFagraad,'.Rmd'),
                    output_format =switch(
                      input$formatReportFagraad,
                      PDF = pdf_document(), HTML = html_document(), Word = word_document()),
                    params = list(dateFrom = input$dateFrom,
                                  dateTo = input$dateTo)
      )
      file.rename(out, file)
    }
  )
  ###-----PDF report fagraad (END)

  ###---PDF-report, minimal for testing if report download fails for other
  ###reports
  #https://shiny.rstudio.com/articles/generating-reports.html
  #and I used these to solve an issue with temp reports:
  #https://mastering-shiny.org/action-transfer.html#downloading-reports,together
  #with Yihui's comment here, and code in test app:
  #https://github.com/rstudio/shiny/issues/1831
